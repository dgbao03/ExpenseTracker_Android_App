    ...
    public boolean checkUserPassword(UserModel user) {
        ...
        if (c.getCount() > 0) {
            ...
            if (passwordColumnIndex != -1) {
                String storedHashedPassword = c.getString(passwordColumnIndex);
                String inputPasswordHash = hashPassword(user.getPassword());

                if (inputPasswordHash != null && inputPasswordHash.equals(storedHashedPassword)) {
                    checkPass = true;
                }
            }
        }
